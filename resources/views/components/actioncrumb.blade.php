                    ...
                    <div class="flex flex-shrink-0 items-center">
                        @if ($step->hasActions() && $config->isDropdownsEnabled())
                            ...
                            <div class="flex flex-shrink-0 items-center {{ $config->getStepContainerClasses($step->isCurrent()) }}">
                                ...
                                @php
                                    $visibleActions = collect($step->getActions())->filter(fn($a) => $a->isVisible());
                                    $hasVisibleActions = $visibleActions->isNotEmpty();
                                @endphp

                                {{-- Dropdown arrow (separate from link) - only show if there are visible actions --}}
