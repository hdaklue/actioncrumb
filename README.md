...
**Requirements:** PHP 8.2+, Laravel 11+, Livewire 3+, Tailwind CSS, Alpine.js
...
**Breadcrumb Navigation Components** - Use `HasCrumbSteps` trait:
```php
use Hdaklue\Actioncrumb\Components\WireCrumb;
...
class UserManagement extends WireCrumb
{
    protected function crumbSteps(): array
    {
        return [
            ...
            WireStep::make(UserStepComponent::class, ['user' => $this->user]),
        ];
    }
}
```

**Individual Step Components** - Use `HasActionCrumbs` trait:
```php
...
### WireStep - Embed Livewire Components

```php
use Hdaklue\Actioncrumb\Components\WireStep;

// In your WireCrumb component
protected function crumbSteps(): array
{
    return [
        Step::make('Dashboard')->url('/dashboard'),

        // Embed a Livewire component as a step
...
class UserStepComponent extends Component
{
    ...
    // For WireStep components, return array of Action objects directly
    protected function actioncrumbs(): array
    {
        return [
            Action::make('Edit')
    ...
    {
        return view('user-step-component');
    }
}
```

**user-step-component.blade.php:**
```blade
<div class="flex flex-shrink-0 items-center">
    <div class="...">
        <span class="...">
            <x-icon name="heroicon-o-user" class="..." />
            {{ $user->name }}
        </span>
    </div>

    {{-- Renders the dropdown arrow and actions --}}
